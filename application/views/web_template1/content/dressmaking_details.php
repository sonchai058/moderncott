<?php 
	...
	$rows = $this->common_model->custom_query(" SELECT * FROM product WHERE P_ID = '$P__ID' AND P_type = '2' "); 
	if (count($rows) > 0) $row = rowArray($rows); else redirect('main/product_details/'.$P__ID, 'refresh');
			...
			<div class="product_add_to_cart"> 
				<?php 
					...
					if (isset($price['PS_FullSumPrice']) && $price['PS_FullSumPrice'] !== null) {
						...
						$price_ini = $price['PS_FullSumPrice'];
					}
					else {
				...
				<table>
					...
					<tr>
						<td>ขนาด:</td>
						<td colspan="1"><?php echo form_dropdown('size', $size_input, '', 'id="size"'); ?></td>
					</tr>
					<tr>
						<td>ความยาว:</td>
						<td colspan="1"><?php echo form_input(array('type' => 'number', 'id' => 'ex_length', 'name' => 'ex_length', 'min' => '34', 'value' => '34')); ?></td>
					</tr>
					<tr>
						<td>สี:</td>
						<td colspan="1"><?php echo form_dropdown('color', $color_input, '', 'id="color"'); ?></td>
					</tr>
					<tr>
						<td>จำนวน:</td>
						<?php if (!isset($price['PS_Amount']) || $price['PS_Amount'] === null || $price['PS_Amount'] < 1) $maxQty = 1; else $maxQty = $price['PS_Amount']; ?>
						<td colspan="1"><?php echo form_input(array('type' => 'number', 'id' => 'qty', 'name' => 'qty', 'value' => '1', 'min' => '1', 'max' => $maxQty, 'required' => 'required')); ?></td>
					</tr>
					<tr>
						<td>หมายเหตุ:</td>
						<td colspan="1"><?php echo form_textarea(array('id' => 'note', 'name' => 'note', 'rows' => '3')); ?></td>
					</tr>
					<tr>
						<td colspan="2"><button class="fancybox"><i class="fa fa-scissors"></i><span>เพิ่มสินค้าสั่งตัด ในตะกร้า</span></button></td>
					</tr>
				</table> <?php
				echo form_close(); ?>	
			</div>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$(".product_add_to_cart").find('button').click(function() {
			var ex_length = '';
			if ($('#ex_length').val() > 38) 
				ex_length = ' (เพิ่มความยาวเป็น ' + $('#ex_length').val() + '\x22)';
			else
				ex_length = '';
			$("#note").val($("#note").val() + ' ');
  			var request = $.ajax({
  				...
  				data 	: { 
  					...
  					qty 	: $("#qty").val(),
  					code 	: $("#code").val(),
  					note 	: $("#note").val() + '(สั่งตัด)' + ' ความยาว:' + $("#size option:selected").text() + ex_length + ' / สี: ' + $("#color option:selected").text(),
  					isDMK	: 1,
  				}
			});
		...
		$("#qty").keyup(function() {
			...
			// if ($("#ex_length").val() > 38) {
				// $("#price").val(parseFloat('<?php echo $price_ini; ?>') + parseFloat($("#ex_price").val()));
			...
			$("#price").val(parseFloat('<?php echo $price_ini; ?>'));
			if ($("#ex_length").val() > 38)
				$("#price").val(parseFloat('<?php echo $price_ini; ?>') + parseFloat($("#ex_price").val()));
			else if ($("#ex_length").val() >= 35 && $("#ex_length").val() <= 38)
				$("#price").val(parseFloat('<?php echo $price_ini; ?>'));
