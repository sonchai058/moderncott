<?php 
	...
	$rows = $this->common_model->custom_query(" SELECT * FROM product WHERE P_ID = '$P__ID' AND P_type = '1' "); 
	if (count($rows) > 0) $row = rowArray($rows); else redirect('main/dressmaking_details/'.$P__ID, 'refresh');
