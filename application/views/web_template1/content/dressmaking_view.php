<?php 
	// $tab_menu = array('recommend', 'popular', 'bestseller'); 
	$tab_menu = array('category_01', 'category_02', 'category_03', 'category_04', 'category_05'); 
?>
<!-- <div class="wrap_content_tab"> -->
<div class="wrap_content_tab_DMK">
	<div class="wrap_content_tab_head">
		<img src="<?php echo base_url('assets/images/all-product-icon.png'); ?>" alt="">
		<i class="fa fa-chevron-right"></i>
		<h1>
			<span>สินค้าสั่งตัด</span>&nbsp;
			<span>(ทั้งหมด)</span>
		</h1>
	</div> 
	<!-- <div id="everything" class="wrap_content_tab_menu active"><a>ทั้งหมด ></a></div>  -->
	<div id="everything" class="wrap_content_tab_menu_DMK active"><a>ทั้งหมด ></a></div> 
	<?php 
	// $row = $this->common_model->getTableOrder('product_type', 'PT_Order', 'ASC'); 
	// $row = $this->common_model->getTableOrder('category', 'C_ID', 'ASC');
	$row = $this->common_model->custom_query(" SELECT * FROM category ORDER BY C_ID ASC LIMIT 5 ");
	if (count($row) > 0) { 
		foreach ($row as $key => $value) { ?>
			<!-- <div id="<?php echo $tab_menu[$key]; ?>" class="wrap_content_tab_menu"><a><?php echo $value['PT_Name']; ?> ></a></div>  -->
			<div id="<?php echo $tab_menu[$key]; ?>" class="wrap_content_tab_menu_DMK"><a><?php echo $value['C_Name']; ?> ></a></div> 
			<?php
		}
	} 
	else { ?>
		<!-- <div id="<?php echo $tab_menu[0]; ?>" class="wrap_content_tab_menu active"><a>ทั้งหมด</a></div>  -->
		<div id="<?php echo $tab_menu[0]; ?>" class="wrap_content_tab_menu_DMK active"><a>ทั้งหมด</a></div> 
		<?php
	} ?>
</div> 
<!-- <div class="wrap_content_grid">  -->
<div class="wrap_content_grid_DMK"> 
<?php
	$tabs = 0;
	...
	<div id="wrap_content_everything" class="wrap_content_row"> <?php 
		...
		$rows = $this->common_model->custom_query(" SELECT * FROM product WHERE P_Allow = '1' AND P_Type = '2' ORDER BY PT_ID ASC, P_DateTimeUpdate DESC "); 
		foreach ($rows as $key => $value) { 
	        ...
	        <div class="wrap_content_col">
				...
				<div class="wrap_cart_or_detail">
					...
					<div>
						<button title="รายละเอียด" onclick="location.href='<?php echo base_url('main/dressmaking_details/'.$value['P_ID']); ?>';"><i class="fa fa-search"></i>รายละเอียด</button>
					</div>
				</div>
			</div> <?php 
		} ?>
	...
	while ($tabs < count($row)) { ?>
	    <div id="wrap_content_<?php echo $tab_menu[$tabs]; ?>" class="wrap_content_row" <?php if ($tabs > 0) { ?> style="display:none;" <?php } ?>> <?php 
			...
			// $rows = $this->common_model->custom_query(" SELECT * FROM product WHERE C_ID = '$PT_ID' AND P_Allow = '1' ORDER BY PT_ID ASC, P_DateTimeUpdate DESC "); 
			$rows = $this->common_model->custom_query(" SELECT * FROM product WHERE C_ID = '$PT_ID' AND P_Allow = '1' AND P_Type = '2' ORDER BY PT_ID ASC, P_DateTimeUpdate DESC "); 
			foreach ($rows as $key => $value) { 
				if ($value['P_Img'] != '')
					list($width, $height) = getimagesize(base_url('assets/uploads/user_uploads_img/'.$value['P_Img']));
				else
	...
	$(document).ready(function() {
		$(".wrap_content_col").mouseover(function() {
			$(this).find(".wrap_cart_or_detail").fadeIn("fast", function() {
				$(this).find("div").animate({ display: 'table' }, 0);
				$(this).find("div").animate({ marginRight: '94px', opacity: 1 }, 250);
  			...
  			$(this).find(".wrap_cart_or_detail").fadeOut("fast", function() {
  				$(this).find("div").animate({ marginRight: '-64px', marginLeft: '-64px', opacity: 0 }, 250);
  				$(this).find("div").animate({ display: 'none' }, 0);
