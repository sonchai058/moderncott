...
class Main extends CI_Controller {
  ...
  //       $this->_example_output($output, $title);
	...
	public function dressmaking_view() {
		if (!isset($_SESSION['visited'])) {
			$visit_data = array(
				'S_IP' 			=> $this->admin_model->kh_getUserIP(),
				'S_UserAgent' 	=> $this->admin_model->get_http_user_agent(),
		...
		$data = array(
			'content_view' 	=> 'content/dressmaking_view',
			'title'			=> 'Dressmaking'
		);
		$this->template->load('index_page', $data);
	}

	public function dressmaking_details($P_ID = null) {
		if (!isset($_SESSION['visited'])) {
			$visit_data = array(
				'S_IP' 			=> $this->admin_model->kh_getUserIP(),
				'S_UserAgent' 	=> $this->admin_model->get_http_user_agent(),
		...
		if ($P_ID !== null && $P_ID !== '') {
			...
			if (count($row) > 0) {
				$data = array(
					'content_view' 	=> 'content/dressmaking_details',
					'title'			=> 'Dressmaking Details',
					'P_ID' 			=> $P_ID
				);
				$this->template->load('index_page', $data);
			}
