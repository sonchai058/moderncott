...
var tab_DMKS 		= ['everything', 'category_01', 'category_02', 	'category_03', 'category_04', 'category_05'];
...
$(document).ready(function() {
	...
	$('.wrap_content_tab_menu_DMK').each(function(i) {
		$('#' + tab_DMKS[i]).click(function() {
			setTimeout(function() {
				wrap_content = $('#wrap_content_' + tab_DMKS[i]).prop('id');

				var tab_index = 0;
				while (tab_index < tab_DMKS.length) {
					if (tab_index !== i) {
						$('#' + tab_DMKS[tab_index]).removeClass('active');
						$('#wrap_content_' + tab_DMKS[tab_index]).css('display', 'none');
					}
					tab_index += 1;
				}

				$('#' + tab_DMKS[i]).addClass('active');
				$('#wrap_content_' + tab_DMKS[i]).fadeIn("fast", function() {
					$('#wrap_content_' + tab_DMKS[i]).css('display', 'inline-block');
				});
				
				number_of_items = $('#wrap_content_' + tab_DMKS[i]).children().size(); 
				number_of_pages = Math.ceil(number_of_items/show_per_page);

				navigation_html = '';
				render_pagination();

				$('#wrap_content_' + tab_DMKS[i]).children().css('display', 'none');
				$('#wrap_content_' + tab_DMKS[i]).children().slice(0, show_per_page).css('display', 'inline-block');
			}, 100);
		});
	});

	$('.wrap_content_tab_DMK').find($('#' + tab_DMKS[0] + ' a')).trigger('click');
	...
	$('.wrap_content_grid_DMK').find($('.wrap_content_pagination')).load(function() {
		wrap_content = $('#wrap_content_pagination').prop('id');
	...
	$('.wrap_content_grid_DMK').find($('.wrap_content_pagination')).trigger('load');
